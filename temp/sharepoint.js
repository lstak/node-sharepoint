...
var getCookie = function (cookies, name) {
    var cookie, i = 0, len = cookies.length;
    for (; i < len; i++) {
        cookie = cookies[i];
        if (cookie.name == name) {
            return cookie;
...
function request(options, next) {
    var req_data = options.data || '', list = options.list, id = options.id, query = options.query, ssl = (this.protocol == 'https:'), path = this.path + this.odatasvc + list +
        (id ? '(' + id + ')' : '') +
